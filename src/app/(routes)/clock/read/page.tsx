...
// 시계 컴포넌트
const ClockDisplay = ({ time }: { time: ClockTime }) => {
  const hourAngle = (time.hour % 12) * 30 + (time.minute / 60) * 30;
  const minuteAngle = time.minute * 6;

  return (
    <div className="relative w-64 h-64 mx-auto">
      {/* 시계 외곽 */}
      <div className="absolute inset-0 rounded-full border-8 border-gray-800 bg-white shadow-lg">
        {/* 시계 숫자 */}
        ...
        {/* 시침 */}
        <div
          className="absolute w-1 h-16 bg-gray-800 rounded-full origin-bottom"
          style={{
            left: "50%",
            top: "50%",
        ...
        {/* 분침 */}
        <div
          className="absolute w-0.5 h-20 bg-gray-800 rounded-full origin-bottom"
          style={{
            left: "50%",
            top: "50%",
...
export default function ClockReadPage() {
  ...
  const generateNewQuestion = useCallback(() => {
    const newTime = generateClockTime(difficulty);
  ...
  const checkAnswer = () => {
    ...
    if (isCorrect) {
      ...
      handleCorrectAnswer("clock_read", difficulty);
    } else {
      handleIncorrectAnswer("clock_read");
    }
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 p-4">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="flex items-center gap-4 mb-8">
          ...
          <h1 className="text-3xl font-bold text-gray-800">시계 보기 게임</h1>
        </div>
          ...
          <div className="bg-white rounded-2xl shadow-lg p-8">
            ...
            <div className="bg-blue-50 rounded-lg p-4 mb-6">
              <div className="text-center">
                <div className="text-sm text-blue-600 mb-1">선택한 시간</div>
                <div className="text-2xl font-bold text-blue-800">
                  {selectedHour}시 {selectedMinute.toString().padStart(2, "0")}
                  분
                </div>
              </div>
            </div>
            ...
            <button
              ...
              className="w-full py-4 bg-green-500 text-white rounded-lg font-semibold text-lg hover:bg-green-600 disabled:bg-gray-300 disabled:cursor-not-allowed transition-colors"
            >
              {showFeedback === null
                ? "정답 확인"
              ...
              >
                {showFeedback === "correct" ? (
                  <div>
                    ...
                    <div className="text-sm">정확한 시간을 읽었어요!</div>
                  </div>
                ) : (
                  <div>
                    <div className="text-lg font-semibold">틀렸어요 😅</div>
                    <div className="text-sm">
                      정답: {formatTime(currentTime)}
                    </div>
                  </div>
                )}
              </div>
